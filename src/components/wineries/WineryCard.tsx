...
export function WineryCard({ winery }: WineryCardProps) {
  return (
    <Link to={publicRoutes.wineryDetail(winery.slug)} className="block overflow-hidden rounded-lg border border-stone-200 bg-cream-50 shadow-sm">
      <img src={winery.coverImageUrl} alt="" className="h-48 w-full object-cover" loading="lazy" />
      <div className="space-y-4 p-5 text-center">
        <img src={winery.logoUrl} alt={`Logo ${winery.name}`} className="mx-auto h-16 max-w-36 object-contain" loading="lazy" />
