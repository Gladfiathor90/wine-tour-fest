...
export function AdminSponsorsPage() {
  ...
  usePageMeta('Admin sponsor', 'Gestione sponsor Wine Tour Fest.')
  ...
  async function handleDelete() {
    if (!id || !window.confirm('Eliminare questo sponsor?')) return
    setSaving(true)
    setMessage('')
    setError('')
    try {
  ...
  async function uploadLogo(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setError('Upload logo non riuscito. Verifica le policy Storage del bucket sponsors.')
    } finally {
      setUploading(false)
      event.target.value = ''
    }
    ...
    return (
      <div className="space-y-5">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-stone-950">Sponsor</h1>
        ...
        <Link to={adminRoutes.newSponsor} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-wine-700 px-4 text-sm font-semibold text-white">
          <Plus className="h-4 w-4" /> Aggiungi sponsor
        </Link>
        {loading ? <p className="rounded-lg bg-white p-5 text-sm font-semibold text-stone-600 shadow-sm">Caricamento sponsor...</p> : null}
        {!loading && !error && !sponsors.length ? <EmptyState icon={Plus} title="Nessuno sponsor" description="Aggiungi il primo logo sponsor." /> : null}
            ...
            <article key={sponsor.id} className="rounded-lg bg-white p-4 shadow-sm">
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-stone-600">{sponsor.level}</p>
              ...
              <div className="mt-3 grid gap-2">
                ...
                {sponsor.active ? <Link className="min-h-11 rounded-md bg-cream-100 px-3 py-3 text-center text-sm font-semibold text-wine-700" to={publicRoutes.sponsors}>Anteprima pubblica</Link> : null}
              </div>
            </article>
          ))}
        </div>
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-sm font-semibold uppercase tracking-[0.16em] text-wine-700">Sponsor</p>
        <h1 className="mt-2 text-3xl font-bold text-stone-950">{isNew ? 'Nuovo sponsor' : 'Modifica sponsor'}</h1>
        <p className="mt-2 text-sm text-stone-600">I loghi pubblicati compaiono nella pagina Sponsor.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded-lg bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-3">
          <button type="submit" disabled={saving} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-wine-700 px-4 text-sm font-semibold text-white disabled:opacity-60">
            <Save className="h-4 w-4" /> {saving ? 'Salvataggio...' : 'Salva sponsor'}
          </button>
          {!isNew ? (
            <button type="button" disabled={saving} onClick={handleDelete} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-red-50 px-4 text-sm font-semibold text-red-700 disabled:opacity-60">
              <Trash2 className="h-4 w-4" /> Elimina sponsor
            </button>
          ) : null}
